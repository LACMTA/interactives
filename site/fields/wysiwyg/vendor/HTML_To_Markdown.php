...
class HTML_To_Markdown
{
    ...
    private function convert_image($node)
    {
        ...
        $displaytitle = $node->getAttribute('title');

        if ($displaytitle != "") {
            $markdown = '![' . $alt . '](' . $src . ' "' . $displaytitle . '")'; // No newlines added. <img> should be in a block-level element.
        } else {
            $markdown = '![' . $alt . '](' . $src . ')';
        }
    ...
    private function convert_anchor($node)
    {
        ...
        if ($displaytitle != "") {
            $markdown = '[' . $text . '](' . $href . ' "' . $displaytitle . '")';
        } else {
            $markdown = '[' . $text . '](' . $href . ')';
        }
